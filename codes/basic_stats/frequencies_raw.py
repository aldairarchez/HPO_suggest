...
#-----------------output directories------
output_directory='/root/HPO_project/HPO_new/New_analysis/Frequency_Raw'
...
#----------------------reformating the Pubcase finder queries archive--------------------
import re

def reformat_rows(line):
    if "[pid:" in line and "phenotype=" in line:
        # Extract the part after "phenotype="
        phenotype_part = line.split("phenotype=")[1]
        
        # Split by either '&' or comma, and space can also be a delimiter
        phenotype_values = re.split(r'[&, ]', phenotype_part)
        
        # Remove "_ja" from each value and strip any extra spaces
        cleaned_values = [value.replace("_ja", "").strip() for value in phenotype_values]
        
        # Check if there are HPO terms starting with "HP"
        valid_values = [value for value in cleaned_values if value.startswith("HP")]
        
        if valid_values:
            # Join valid values with commas and remove extra quotes or newlines
            return ','.join(valid_values).strip('"').strip()
    
    return None



# Read the file and process lines in one pass
with open(f'{query_file}', 'r') as file:
    reformatted_lines = [reformat_rows(line) for line in file if reformat_rows(line) is not None] #eliminates those queries that does not have hpo terms
    #print(reformatted_lines)
# Create a DataFrame from the reformatted lines
df_queries = pd.DataFrame(reformatted_lines, columns=['Reformatted Rows'])

# Clean the DataFrame by removing extra newlines and quotes
df_queries['Reformatted Rows'] = df_queries['Reformatted Rows'].str.strip().str.replace('"', '', regex=False)

# Save the DataFrame to a CSV file, with escape character for special cases
df_queries.to_csv(f'{output_directory}/queries.csv', sep=',', index=False, header=False, escapechar='\\')

# Read the CSV file into a DataFrame
df = pd.read_csv(f'{output_directory}/queries.csv', sep=',', header=None)

#some lines has ';' so i'm going to elinate
#bash_command = f"sed 's/;/,/g' {output_directory}/queries.csv > {output_directory}/queries_cleaned.csv"
...
# Read the CSV file into a DataFrame
df = pd.read_csv(f'{output_directory}/queries_cleaned.csv', sep=',', header=None)
...
df_queries2 = pd.read_csv(f'{output_directory}/queries_cleaned.csv', header=None)
